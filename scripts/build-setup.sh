...
conda environment or \`source env.sh\` and skip this step with \`-s 1\`." >&2
    ...
    conda create -y -p $CONDA_LOCK_ENV_PATH -c conda-forge python=3.10 $(grep "conda-lock" $CONDA_REQS/chipyard-base.yaml | sed 's/^ \+-//') &&
    ...
    if [ "$SYS_GLIBC" != "$DEFAULT_GLIBC" ]; then
        # Choose the nearest available sysroot version that does not exceed host glibc.
        # This avoids pinning to versions that have been removed from upstream channels.
        MATCHED_GLIBC=$(python - "$SYS_GLIBC" <<'PY'
import json
import subprocess
import sys
...
try:
    out = subprocess.check_output(
        ["conda", "search", "-c", "conda-forge", "--override-channels", "sysroot_linux-64", "--json"],
        text=True,
    )
    data = json.loads(out)
    versions = sorted({pkg["version"] for pkg in data.get("sysroot_linux-64", [])}, key=parse_ver)
    candidates = [v for v in versions if parse_ver(v) <= target]
...
PY
        ...
        if [ -n "$MATCHED_GLIBC" ]; then
            echo "Adjusting sysroot_linux-64 from $DEFAULT_GLIBC to $MATCHED_GLIBC (host glibc: $SYS_GLIBC)."
            sed -i.bak "s/^\([[:space:]]*-\s*sysroot_linux-64=\).*/\1$MATCHED_GLIBC/" conda-reqs/chipyard-base.yaml
        else
            echo "Warning: could not find an available sysroot_linux-64 <= host glibc ($SYS_GLIBC); keeping configured value $DEFAULT_GLIBC." >&2
        fi
        $CYDIR/scripts/generate-conda-lockfiles.sh
        exit_if_last_command_failed
    fi
...
if run_step "8"; then
    ...
    $CYDIR/scripts/firemarshal-setup.sh --phase init --cydir "$CYDIR"
    exit_if_last_command_failed

    # precompile firemarshal buildroot sources
    if run_step "9"; then
        begin_step "9" "Pre-compiling FireMarshal buildroot sources"
        $CYDIR/scripts/firemarshal-setup.sh --phase precompile --cydir "$CYDIR" --verbose-flag "$VERBOSE_FLAG"
        exit_if_last_command_failed
    fi
fi
